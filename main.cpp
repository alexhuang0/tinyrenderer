...
int main(int argc, char** argv) {
	...
	*/
	...
	matrix<4, 4> camera_to_model{ (rContext.Viewport * rContext.Perspective * rContext.ModelView).inverse() };
...
#pragma omp parallel for
		for (std::size_t x{ 0 }; x < width; ++x) {
			for (std::size_t y{ 0 }; y < height; ++y) {
				vec4 cameraCoord{ x, y, rContext.zbuffer[x + y * width], 1. };
				vec4 lightCoord{ camera_to_model * cameraCoord }; // coord on the OBJECT, seen from the light
				// shContext.zbuffer is closest obj the light can see
				constexpr double shadow_bias{ 0.03 };

				if (lightCoord.z + shadow_bias >= shContext.zbuffer[x + y * width]) {
					// add shadow bias to shContext because: suppose the a coord is testing against itself
					// -> which condition makes it so that it ISN'T considered to be in the shadow? (prevent self-shadow)

					// add 1, the fragment is visible
