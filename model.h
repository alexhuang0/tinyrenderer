...
public:
	...
	vec4 vert(const std::size_t i) const; // 0 <= i < nverts()
	vec4 vert(const std::size_t iface, const std::size_t nthvert) const; // 0 <= iface < nfaces(), 0 <= nthvert < 3

	vec4 normal(const std::size_t iface, const std::size_t nthnorm) const; // normal coming from "vn x y z"
